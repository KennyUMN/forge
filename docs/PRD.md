...
- **Known dependency vulnerabilities, accepted as unreachable (2026-07-22)**: `npm audit` reports 7 advisories — 1 critical/1 high in the `vite`/`vitest`/`esbuild` devDependency toolchain (pre-existing since Sprint 1, only exploitable via Vitest's interactive `--ui` mode, which Forge's scripts never invoke — `vitest run` only) and 1 moderate in `@hono/node-server`, pulled in transitively by `@modelcontextprotocol/sdk` for its optional HTTP/SSE server-side transport (Sprint 3 only uses the client side via stdio transport, so this code path is never reached either). No non-breaking fix exists for either as of this date — `npm audit fix --force` would downgrade the MCP SDK to 1.24.3 and bump vitest to a new major version. Decision: leave as-is rather than take an unplanned breaking-change bump mid-sprint; revisit when a non-breaking fix ships, or before any deployment scenario where `vitest --ui` or the MCP SDK's HTTP transport might actually be exposed.
