...
describe("LocalExecutor", () => {
  ...
  describe("executeCommand", () => {
    it("runs a command and captures stdout", async () => {
      const result = await executor.executeCommand("echo hello", { cwd: "/tmp" });
      expect(result.stdout.trim()).toBe("hello");
      expect(result.exitCode).toBe(0);
    });

    it("captures stderr", async () => {
      const result = await executor.executeCommand("echo err-msg >&2", { cwd: "/tmp" });
      expect(result.stderr.trim()).toBe("err-msg");
      expect(result.exitCode).toBe(0);
    });

    it("captures non-zero exit codes", async () => {
      const result = await executor.executeCommand("exit 42", { cwd: "/tmp" });
    ...
    it("runs in the specified cwd", async () => {
      ...
      try {
        ...
        const result = await executor.executeCommand("pwd", { cwd: dir });
    ...
    it("respects abort signal", async () => {
      ...
      controller.abort();
      await expect(
        executor.executeCommand("sleep 10", { cwd: "/tmp", signal: controller.signal }),
      ).rejects.toThrow("Aborted");
    });

    it("aborts a running command", async () => {
      const controller = new AbortController();
      const promise = executor.executeCommand("sleep 30", { cwd: "/tmp", signal: controller.signal });
    ...
    it("rejects on timeout", async () => {
      await expect(
        executor.executeCommand("sleep 10", { cwd: "/tmp", timeout: 100 }),
